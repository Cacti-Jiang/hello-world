...
write("Identify clusters of cells by Seurat
Examples:
...
ParaSet = matrix( c(
	...
	"rmlist", "t", 2, "character", "input sample names that should be removed",
...
Para = getopt(ParaSet);
if (is.null(Para$input)) { Func_OutUsage(ParaSet);}
...
Obj <- FindClusters(Obj, reduction.type = "pca",dims.use = c(Para$dim), save.SNN = T, 
                    k.param = Para$k.param, resolution = Para$resolutiona)

Obj <- RunTSNE(Obj, dims.use = c(Para$dim), do.fast = T, perplexity = 5)
